...
export abstract class BaseAgent {
  ...
  async execute(context: AgentContext, action: ActionType, payload: unknown): Promise<AgentResult> {
    ...
    try {
      ...
      await averecionClient.reportDecision(
        context.agentId,
        effectiveAction,
        { query: context.userQuery, ...(typeof payload === 'object' && payload !== null ? payload : {}) },
  ...
  private getTaskDescription(action: ActionType): string {
    const taskDescriptions: Record<string, string> = {
      ...
      triage_claim: "Classify claim severity and score priority",
      analyze_backlog: "Analyze claims backlog metrics and throughput",
      manage_adjusters: "Optimize adjuster assignments and workload",
      assess_storm: "Assess storm impact and estimate losses",
    };
  ...
  private getDecisionRationale(action: ActionType, context: AgentContext): string {
    const rationales: Record<string, string> = {
      ...
      triage_claim: "Claim classification or severity assessment keywords detected",
      analyze_backlog: "Backlog metrics or queue analysis requested",
      manage_adjusters: "Adjuster assignment or workload balancing requested",
      assess_storm: "Storm impact analysis or loss estimation requested",
    };
  ...
  private getRiskLevel(action: ActionType): "low" | "medium" | "high" | "critical" {
    const riskLevels: Record<string, "low" | "medium" | "high" | "critical"> = {
      query_response: "low",
      triage_claim: "medium",
      analyze_backlog: "medium",
      manage_adjusters: "high",
      assess_storm: "high",
    };
  ...
  private async handleHighRiskAction(
    ...
    const actionContexts: Record<string, { goal: string; context: string; isIrreversible: boolean }> = {
      ...
      assess_storm: {
        goal: "Assess storm impact and estimate losses",
        context: `Storm: ${payloadData.stormId || context.userQuery}`,
        isIrreversible: false,
      },
      manage_adjusters: {
        goal: "Optimize adjuster assignments",
        context: `Region: ${payloadData.region || context.userQuery}`,
        isIrreversible: false,
      },
    };
  ...
  private getAlternativeActions(chosenAction: ActionType): Array<{
    ...
    const actionAlternatives: Record<string, Array<{ action: string; confidence: number; reason_not_chosen: string }>> = {
      triage_claim: [
        { action: "query_response", confidence: 0.3, reason_not_chosen: "User query specifically mentions claim triage or severity" },
        { action: "assess_storm", confidence: 0.2, reason_not_chosen: "Claim triage is more appropriate than storm assessment" },
      ],
      analyze_backlog: [
        { action: "query_response", confidence: 0.25, reason_not_chosen: "User explicitly requested backlog analysis" },
        { action: "manage_adjusters", confidence: 0.4, reason_not_chosen: "Backlog analysis is requested, not adjuster management" },
      ],
      manage_adjusters: [
        { action: "query_response", confidence: 0.2, reason_not_chosen: "Adjuster management specifically requested" },
      ],
      assess_storm: [
        { action: "triage_claim", confidence: 0.5, reason_not_chosen: "User asking about storm impact, not individual claim triage" },
        { action: "query_response", confidence: 0.15, reason_not_chosen: "Storm assessment requires specialized processing" },
      ],
    };
